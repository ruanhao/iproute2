...
static void show_slabstat(struct slabstat* s) {
    int ids_size = sizeof(slabstat_ids) / sizeof(slabstat_ids[0]);
    int i;
    for (i = 0; i < ids_size; i++) {
        H_DEBUG_MSG("%s: %d", slabstat_ids[i], *((int *)s + i));
    }
}
...
static void get_sockstat_line(char *line, struct sockstat *s)
{
    ...
    if (strcmp(id, "sockets:") == 0)
        ...
        sscanf(rem, "%*s%d%*s%d%*s%d%*s%d%*s%d",
               &s->tcp4_hashed, &s->tcp_orphans, &s->tcp_tws, &s->tcp_total, &s->tcp_mem);
}
